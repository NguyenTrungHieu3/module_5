...
function StudentListPage() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                const majorsData = await getAllMajor();
    ...
    const filteredStudents = students.filter((student) => {
        const matchName = searchText === '' || student.name.toLowerCase().includes(searchText.toLowerCase());
        const matchMajor = selectedMajor === '' || student.major.id === Number(selectedMajor);
    ...
    const handleDeleteStudent = async () => {
        const isSuccess = await deleteStudentById(deleteStudent.id);
        if (isSuccess) {
            setReloading(!reloading); // Báo cho useEffect tải lại danh sách
        }
        setIsShowDeleteModal(false);
    };
